...
object Star8 {
  def solve(): Unit = {
      ...
      for(boardIdx <- 0 until boards.length / 25) {
        ...
        for(y <- 0 until 5) {
          if((0 until 5).map(x => boards(boardIdx * 25 + 5 * y + x) < 0).reduce((agg, memb) => agg && memb)) {
            nWins += 1
            if(nWins == (boards.length / 25)) {
              lastWin = boardIdx
              lastNumber = n
              break
            } else for(i <- 0 until 25) {
              boards(boardIdx * 25 + i) = 100.asInstanceOf[Byte]
            }
          }
        }
        for(x <- 0 until 5) {
    ...
    assert(lastWin >= 0 && lastNumber >= 0)
    var score = 0
    for(y <- 0 until 5) {
      for(x <- 0 until 5) {
        if(boards(lastWin * 25 + 5 * y + x) >= 0) score += boards(lastWin * 25 + 5 * y + x)
      }
    }
    println("Star8 Answer: %d".format(score * lastNumber))
  }
}