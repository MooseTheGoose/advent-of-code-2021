...
object Star15 {
    ...
    def discoverOutput(input: Array[String], output: Array[String]): Int = {
      ...
      for(i <- input) {
          if(i.length() == 6) {
              if(!i.contains(wirings(1)(0)) || !i.contains(wirings(1)(1))) {
                wirings(6) = i
              }
          }
      }
      ...
      /*
       * We can finally finish with 0 and 9
       * 9 contains all of 4's wires, whereas 0 does not 
       */
      for(i <- input) {
          if(i.length() == 6) {
              if(wirings(4).filter(c => i.contains(c)).size == 4) wirings(9) = i
              else wirings(0) = i
          }
      }
    ...
    def solve() {
        var fp = Source.fromURL(getClass.getResource("/star15_input.txt"))
        var nDigits = 0
        for(line <- fp.getLines) {
          var (input, output) = parseLine(line)
          var digits = discoverOutput(input, output)
          while(digits > 0) {
              if(digits % 10 == 4 || digits % 10 == 1 || digits % 10 == 7 || digits % 10 == 8) nDigits += 1
              digits /= 10
          }
        }
        fp.close
        println("Star15 Answer: %d".format(nDigits))
    }
}